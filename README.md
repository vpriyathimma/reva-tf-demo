...
## Open items (need Amit / verification)

1. **Staging PDP URL, Policy Store ID, Auth Token** — go in the TF dashboard
   `config` (or `.env` for local).
2. **User properties source (mechanism known; needs confirmation).** The path
   is the `X-TFY-METADATA` header → `context.metadata` (see section above), so
   the plugin's design is correct. Two things left: (a) Amit to confirm this is
   the intended source vs. a Reva entity lookup; (b) verify on a live TF request
   (with `REVA_DEBUG=1`) that the keys land as `team`/`tier`. This is the
   concrete follow-up to Amit's "do we even get the user's info?" question.
3. ~~**MCP tool-call authorization**~~ — **RESOLVED.** TF does route MCP
   invocations to custom guardrails, via the `mcp_pre_tool` hook. See
   "MCP tool authorization (`mcp_pre_tool`) — verified" above.
4. **Guardrail rules must not be scoped to only-allowed models.** A rule whose
   target is `Models IN [gpt-4o]` never fires for `gpt-4o-mini`, so the gateway
   silently lets the unauthorized model through — the guardrail can only ever
   see calls it would have allowed. Use `Any Model`.
5. **`context.onBehalfOf.active` is not evaluated by the PDP.** A forbid keyed on
   it publishes fine but never fires: the PDP does not hydrate a *context*
   entity's stored attributes, and ignores attributes sent inline. Principal and
   resource entities *are* hydrated. So "deny when the delegating user is
   inactive" cannot currently be expressed on `invokeModel`. Raise with the Reva
   PDP team.
6. **TF does not surface the plugin's `message`.** A rule's
   `{{guardrail_message}}` interpolates TrueFoundry's own generic text
   ("Guardrail violation detected"), not `ValidateGuardrailResponse.message`.
   Reva's real reason is visible in the plugin log and Decision Logs only.

## Design decisions

1. **Validate, not Mutate** — Reva authorizes; it doesn't rewrite content.
2. **Target: input** — authorize before the prompt reaches the model.
3. **Always HTTP 200** — policy denials are `verdict:false`; non-2xx is
   reserved for infra failure. Even unexpected plugin errors are caught and
   converted to a fail-mode verdict.
4. **Faithful port** — eval shape, tier/provider/category heuristics, risk
   baseline, traceparent, and decision parsing are lifted from the LiteLLM
   hook so both gateways are indistinguishable to the PDP.
```
